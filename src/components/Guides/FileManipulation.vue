<template>
  <div class="contents">
    ...
    <p>Often we want to move files around the filesystem, either to organize
      them or because they
      need to be in a specific location. We can do this in a graphical program
      or via the shell.</p>
    <p>Remember you can use relative or absolute <router-link to="/learnlinux/navigation#pathnames" class="bodylink">pathnames</router-link> with all these commands.</p>
    ...
    <p>Mv works by simply changing the pathname of a file. Therefore, we can use
      mv to
      rename files.</p>
    ...
    <p>Sometimes you will want to delete unwanted files or directories. The
      process to remove
      regular files is different from removing directories.</p>
    <p style="color:red">Warning! Using the following methods permanently
      deletes files and
    ...
    <p>Rmdir will not allow you to delete a populated directory- make sure it is
      empty.
      To delete a populated directory, you need to use rm -r.</p>

    <h2>Display text files</h2>
    <p>There are multiple ways to display text files in the terminal window. The
      most common ways
      are explained below.</p>

    <h3 id="cat">cat: Manipulate files</h3>
    <p class="code">$ cat <em>filename</em></p>
    <p>Cat, in its simplest form, displays the contents of a file in the
      terminal window.
    ...
    <p>Press <b>SPACE</b> to scroll down a page and <b>ENTER</b> to scroll down
      one line.</p>

    <h3 id="less">less: A less simple pager</h3>
    <p class="code">$ more <em>filename</em></p>
    <p>Less is a more sophisticated pager, and is the default pager used by
      <router-link to="/learnlinux/permissions#man" class="bodylink">man</router-link>.
    </p>

    <h2>Wildcards and Globbing</h2>
    <p>A notable technique in Linux and other environments is globbing.</p>
    <p>Globbing is the process of using special characters (wildcard characters)
      in commands
      to specify filenames. These wildcard characters can represent multiple
      characters or sets of characters; this
      is extremely useful if:</p>
    <ol>
      <li>You do not know the exact filename</li>
      <li>You want the command to execute on multiple files</li>
    </ol>
    <p>While wildcards could take up a tutorial of their own, the only two that
      you need to know
      about are * and ?</p>

    <h3>* : Any and all characters</h3>
    <p>The asterisk character represents any number of characters, including
      zero characters, in
      a filename.</p>
    ...
    <p>The example command will delete all files in the current directory with
      the .pdf extension.</p>
    ...
    <p>Some example filenames that the example will delete: june1report.txt,
      june_report.txt,
      juneAreport.txt.</p>
    ...
    <p>Some example filenames that the example will delete: hello.aaab,
      linux.aaaz, 123.aaa4.</p>
    ...
    <p>Globbing is extremely useful in many applications including moving
      multiple files, deleting
      multiple files, and more.</p>

    <div class="chapter">
      <div class="previous"><router-link to="/learnlinux/permissions">Previous Tutorial</router-link></div>
      <div class="index"><router-link to="/learnlinux">Index</router-link></div>
      <div class="next"><router-link to="/learnlinux/diskssystem">Next Tutorial</router-link></div>
    </div>
